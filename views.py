...
def Search_Location_DataSets(request):
    if request.method == "POST":
        kword = request.POST.get('keyword')
        if request.method == "POST":
            kword = request.POST.get('keyword')
        CKword=[]
        obj = Crime_type.objects.filter(STREET=kword)
        print("obj", obj)
        for object in obj:
            print("obj",object.CTYPE)
            CKword.append(object.CTYPE)
        print("CKword",CKword)
        unique_values = set(CKword)
        duplicate_values = [item for item in CKword if CKword.count(item) > 1]
        total_items = len(CKword)
        thislist=[]
        try:
            unique_percentage = (len(unique_values) / total_items) * 100
            duplicate_percentage = (len(duplicate_values) / total_items) * 100
            print("Unique names:", unique_values)
            print("Duplicate names:", set(duplicate_values))
            print("Individual names and percentages:")
            for item in unique_values:
                percentage = (CKword.count(item) / total_items) * 100
                print(f"{item}: {percentage:.2f}%")
                thislist.append( percentage)
        except:
            return render(request, 'RUser/Search_Location_DataSets.html',{'error': "e"})
   

        
        return render(request, 'RUser/Search_Location_DataSets.html',{'thislist': thislist,'my_list': CKword, 'objs': unique_values, 'titem': total_items })
    return render(request, 'RUser/Search_Location_DataSets.html')
